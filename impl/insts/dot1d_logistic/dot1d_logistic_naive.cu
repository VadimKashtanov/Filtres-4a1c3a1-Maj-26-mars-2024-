...
static __global__ void kerd_dot1d_logistic_naive(
	...
	float * locd)
{
	...
	if (_t < T && _y < Y) {
		...
		locd[/*(depart+_t)*/(0+_t)*Y + _y] = dot1d_logistic_dACTIV(dot1d_logistic_ACTIVATION, s,a);
	}
};
...
void nvidia_dot1d_logistic_naive(
	...
	float * locd)
{
	kerd_dot1d_logistic_naive<<<dim3(KERD(T, BLOQUE_T), KERD(Y, BLOQUE_Y)), dim3(BLOQUE_T, BLOQUE_Y)>>>(
		_t_MODE, GRAINE,
		X_vars, Y_vars,
		X, Y,
		depart, T,
...
static __global__ void kerd_deriv_dot1d_logistic_naive(
	uint _t_MODE, uint GRAINE,
	uint X_vars, uint Y_vars,
	uint X, uint Y,
	uint depart, uint T,
...
void d_nvidia_dot1d_logistic_naive(
	...
	float * dp)
{
	kerd_deriv_dot1d_logistic_naive<<<dim3(KERD(T, BLOQUE_T), KERD(Y, BLOQUE_Y)), dim3(BLOQUE_T, BLOQUE_Y)>>>(
		_t_MODE, GRAINE,
		X_vars, Y_vars,
		X, Y,
		depart, T,
