...
void cree_dot1d_logistic(Mdl_t * mdl, uint c)
{
	...
	FOR(0, y, Y) {
		FOR(0, x, X+1) {
			mdl->p[c][y*(X+1)+x] = (2*rnd()-1) * sqrtf(/*10.0*/ 6.0 / (X+Y));
		}
	}
};
...
#pragma omp for*/
	FOR(0, t, T) {
		FOR(0, _y, Y) {
			...
			float a = dot1d_logistic_ACTIV(dot1d_logistic_ACTIVATION, s);
			y[(0+t)*Y+_y]    = a;
			locd[(0+t)*Y+_y] = dot1d_logistic_dACTIV(dot1d_logistic_ACTIVATION, s, a);
		}
	}
}
...
void f_dot1d_logistic(Mdl_t * mdl, uint inst, uint mode, uint t0, uint t1, uint _t_MODE, uint GRAINE) {
	...
	if (mode == 0) {
		...
		nvidia_dot1d_logistic_shared_2_16(
			_t_MODE, GRAINE,
			X_vars, Y_vars,
			X, Y,
			depart, T,
...
void df_dot1d_logistic(Mdl_t * mdl, uint inst, uint mode, uint t0, uint t1, uint _t_MODE, uint GRAINE) {
	...
	if (mode == 0) {
		...
		d_nvidia_dot1d_logistic_shared(
			_t_MODE, GRAINE,
			X_vars, Y_vars,
			X, Y,
			depart, T,
		...
		d_nvidia_dot1d_logistic_shared_2_16(
			_t_MODE, GRAINE,
			X_vars, Y_vars,
			X, Y,
			depart, T,
