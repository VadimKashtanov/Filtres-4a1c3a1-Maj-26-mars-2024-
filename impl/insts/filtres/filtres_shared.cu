	...
	float * locd)
{
	...
	FOR(0, plus_t, _repete_T) {
		...
		//
		if (thx != 0) {
			float Pd = (1.0+thx/N*1.0);
			dif_xi = dif_x[LIGNE*PRIXS*N_FLTR + /*(depart+_t)*/cuda_depart_plus_t*N_FLTR + thx];
			atomicAdd(&__ret[thy__t][1], powf((1 + fabs(dif_xi - (fi-fi1))), Pd));
		}
		float Ps = (0.5+thx/N*0.5);
		...
		if (thx < 2) {
			...
			//locd[(0+_t)*BLOQUES*(F_PAR_BLOQUES*2) + BLOQUE*(F_PAR_BLOQUES*2) + _f*2 + thx] = -2*2*__ret[thy__t][thx]*__y[thy__t];
			locd[(0+_t)*BLOQUES*(F_PAR_BLOQUES*2) + BLOQUE*(F_PAR_BLOQUES*2) + _f*2 + thx] = -2*__ret[thy__t][thx]*__y[thy__t]*fp_d_normalisation(__y[thy__t]);
		...
		if (thx < 1) {
			...
			//y[(0+_t)*BLOQUES*F_PAR_BLOQUES + BLOQUE*F_PAR_BLOQUES + _f] = 2*__y[thy__t] - 1;
			y[(0+_t)*BLOQUES*F_PAR_BLOQUES + BLOQUE*F_PAR_BLOQUES + _f] = fp_normalisation(__y[thy__t]);
	...
	float * df)
{
	...
	FOR(0, plus_t, _repete_T) {
		...
		//
		if (thx < 2) {
			__locd[thy__t][thx] = locd[(0+_t)*BLOQUES*(F_PAR_BLOQUES*2) + BLOQUE*(F_PAR_BLOQUES*2) + _f*2 + thx] * __dy0[thy__t]/ (float)(8 - thx);
		}
		__syncthreads();
		//
		xi = x[LIGNE*PRIXS*N_FLTR + /*(depart+_t)*/cuda_depart_plus_t*N_FLTR + thx];
