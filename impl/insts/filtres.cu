...
static float filtre(float * x, float * dif_x, float * f, float * locd) {
	float s = 0, d = 0;
	float f_nouveau = f[0];
	s += powf(1 + fabs(x[0] - f_nouveau), (0.5+0/N*0.5));
...
static void d_filtre(float * x, float * dif_x, float * f, float * locd, float * dy, float * df) {
	...
	FOR(1, i, N)
	{
		...
		df[ i ] += dd * Pd * powf(1 + fabs(dif_x[i] - (f[i]-f[i-1])), Pd-1) * signe(dif_x[i] - (f[i]-f[i-1])) * (-1);
		df[i-1] += dd * Pd * powf(1 + fabs(dif_x[i] - (f[i]-f[i-1])), Pd-1) * signe(dif_x[i] - (f[i]-f[i-1])) * (+1);
