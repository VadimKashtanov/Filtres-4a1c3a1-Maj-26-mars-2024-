...
float * pourcent_masque = de_a(0.05, 0.05, C);			//	Des poids nulls
float * pourcent_masque_opti = de_a(0.20, 0.05, C);		//	Des poids qui ne s'optimiseront pas

float * alpha = de_a(1e-5, 1e-5, C);//de_a(1e-5, 1e-5, C);

uint * optimiser_tous_les = UNIFORME_C(1);

#define GRAND_T (16*16*3)
...
int main(int argc, char ** argv) {
	...
	pourcent_masque_nulle[0] = 0.01;
	//pourcent_masque[0] = 0.01;

	//	----- Lien constants ------

	/*
		* Attention Mechanisme (softmax)
		* f(ax+bx+c)
		* f(ax+b) * f(ax+b) + f(ax+b)
		* f(ax+b) * g(ax+b) + f(ax+b)
	*/
	...
	uint insts[C];
	uint cible[C];
	uint decale_future[C];	// !!!! des vrai donnees aleatoires
	//
	uint st[C][4] = {
		//{4096, DOT1D,          CIBLE_FILTRES_FUTURES,   1 },
		//
	//	  Y      inst             cible             decale-future
		{4096, FILTRES_PRIXS, 			CIBLE_NORMALE, NULL},		//_TOLERANT
		{128,  DOT1D_TANH,             	CIBLE_NORMALE, NULL},
		//
		{128,  DOT1D_TANH,             	CIBLE_NORMALE, NULL},
		{128,  DOT1D_TANH,             	CIBLE_NORMALE, NULL},
		{128,  DOT1D_TANH,             	CIBLE_NORMALE, NULL},
	...
	// -------
		cree_ligne(SRC_PRIXS, RSI, 1, 4, cree_RSI(4)),
		cree_ligne(SRC_PRIXS, RSI, 1, 8, cree_RSI(8)),
		cree_ligne(SRC_PRIXS, RSI, 1, 8, cree_RSI(8)),
		cree_ligne(SRC_PRIXS, RSI, 1, 4, cree_RSI(4)),
		cree_ligne(SRC_PRIXS, RSI, 1, 8, cree_RSI(8)),
		cree_ligne(SRC_PRIXS, RSI, 1, 8, cree_RSI(8)),
		cree_ligne(SRC_PRIXS, RSI, 4, 2.0, cree_RSI(2.0)),
		cree_ligne(SRC_PRIXS, RSI, 4, 4, cree_RSI(4)),
		cree_ligne(SRC_PRIXS, RSI, 16, 16, cree_RSI(16)),
		cree_ligne(SRC_PRIXS, RSI, 16, 16, cree_RSI(16)),
		cree_ligne(SRC_PRIXS, RSI, 4, 4, cree_RSI(4)),
		cree_ligne(SRC_PRIXS, RSI, 16, 16, cree_RSI(16)),
		cree_ligne(SRC_PRIXS, RSI, 16, 16, cree_RSI(16)),
		cree_ligne(SRC_PRIXS, RSI, 16, 128, cree_RSI(128)),
		cree_ligne(SRC_PRIXS, RSI, 16, 64, cree_RSI(64)),
		cree_ligne(SRC_PRIXS, RSI, 16, 128, cree_RSI(128)),
		cree_ligne(SRC_PRIXS, RSI, 16, 128, cree_RSI(128)),
		cree_ligne(SRC_PRIXS, RSI, 16, 128, cree_RSI(128)),
		cree_ligne(SRC_PRIXS, RSI, 32, 128, cree_RSI(128)),
		cree_ligne(SRC_PRIXS, RSI, 64, 64, cree_RSI(64)),
		cree_ligne(SRC_PRIXS, RSI, 64, 256, cree_RSI(256)),
		cree_ligne(SRC_PRIXS, RSI, 256, 128.0, cree_RSI(128.0)),
		cree_ligne(SRC_PRIXS, RSI, 256, 128.0, cree_RSI(128.0)),
		cree_ligne(SRC_PRIXS, RSI, 256, 256, cree_RSI(256))
	};
	//
